# -*- coding: utf-8 -*-
"""
Step 2/2: 실시간 객체 추적 - cv2.KalmanFilter + Occlusion 대응 (최종본)
원본: object_tracking_cv.py (단계별로 재구성)
"""


# %% [markdown]
# ### 2단계: Occlusion(가려짐) 상황 대응
#
# 1단계에서는 객체가 안 보이는 프레임에서 Kalman Filter를 아예 적용하지 않았습니다.
# 이번 단계에서는 측정값이 없을 때도 `kalman.predict()`만 호출해 위치를 계속 추정하도록
# `elif` 분기를 추가합니다. 조명 변화나 순간적인 가려짐에도 추적이 끊기지 않습니다.
#
# 이 파일의 결과는 원본 `object_tracking_cv.py`와 동일합니다.
