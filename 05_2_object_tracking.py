# -*- coding: utf-8 -*-
"""
Step 2/2: 실시간 객체 추적 - 직접 구현한 Kalman Filter + Occlusion 대응 (최종본)
...
#
# 1단계에서는 객체가 안 보이는 프레임에서는 필터를 아예 적용하지 않았습니다.
# 이번 단계에서는 측정값이 없을 때 `z=None`으로 `KalmanFilter()`를 호출해
# 예측만 수행하는 `elif` 분기를 추가합니다.
#
# 이 파일의 결과는 원본 `object_tracking.py`와 동일합니다.
