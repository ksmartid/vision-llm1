# -*- coding: utf-8 -*-
"""
Step 1/2: 실시간 객체 추적 - cv2.KalmanFilter 기본 적용 (Occlusion 미대응)
원본: object_tracking_cv.py (단계별로 재구성)
"""


# %% [markdown]
# ### 1단계: `cv2.KalmanFilter`로 예측/보정 적용
#
# 03_3에서 완성한 색상 기반 객체 검출 위에, OpenCV가 제공하는 `cv2.KalmanFilter`를 붙여
# 매 프레임 측정값(중심 좌표)으로 예측(prediction)과 보정(update)을 수행합니다.
#
# 이 단계에서는 객체가 프레임에서 사라졌을 때(occlusion)는 아직 처리하지 않습니다.
# 즉, 객체가 보이지 않는 프레임에서는 예측을 건너뛰고 마지막 보정값을 그대로 표시합니다.
...
while True:
    ...
    if len(contour_lst) > 0:
        ...
        # (아직 Occlusion 대응 elif 분기는 추가하지 않은 상태입니다 -> 2단계에서 추가)
        if found:
