# -*- coding: utf-8 -*-
"""
Step 1/2: 실시간 객체 추적 - 직접 구현한 Kalman Filter 기본 적용 (Occlusion 미대응)
원본: object_tracking.py (단계별로 재구성)
"""


# %% [markdown]
# ### 1단계: 직접 구현한 `KalmanFilter()` 함수로 예측/보정 적용
#
# `cv2.KalmanFilter` 대신, 선형대수(역행렬/전치/내적)를 직접 사용해 만든 Kalman Filter
# 함수를 붙여봅니다. 이 단계에서는 객체가 보일 때만 필터를 적용하고,
# 객체가 사라졌을 때(occlusion)는 아직 처리하지 않습니다.
